...
s1_l=[0,1,2,3,4,5,6,7]
s2_l=[0,1,2,3,4,5,6,7]
...
##### (source1)
sou1= splitarr(s11, 8, s1_l)
sou1_l=splitarr(s11_l, 8, s1_l)


# In[154]:


print(len(np.unique(sou1_l)))
...
##### (source2)
sou2= splitarr(s22, 8, s2_l)
sou2_l=splitarr(s22_l, 8, s2_l)
#print(sou2_l)
...
pt_all =tf.reduce_mean(tf.cast(tf.equal(tf.argmax(Y3_ind, 1), tf.argmax(p_t, 1)), tf.float32))
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epoch):
        ...
        if p==0:
            ...
            pass_avg2 = sess.run([pt_all], feed_dict={X3:tar, Y3_ind:tar_l_h})
