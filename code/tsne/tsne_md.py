...
# all_features = np.concatenate((market_f,duke_f,d_m_f), axis = 0)
all_features = np.concatenate((market_f,duke_f,m_d_f), axis = 0)

dataset_label = np.zeros((all_features.shape[0],1))

dataset_label[num_market:num_market + num_duke] = 1
dataset_label[num_market + num_duke:] = 2
